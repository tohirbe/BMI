...
import { FileUp, FileCheck, X, AlertCircle, Info, BookOpen, Layers, ChevronRight, ChevronLeft, Save } from "lucide-react";
...
export default function AttendanceBulkPage() {
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-12 max-w-5xl mx-auto">
      <PageHeader 
        title={t('attendance.bulk_title')} 
        subtitle="CSV yoki Excel fayllari orqali davomatni ommaviy yuklash"
        actions={
          <Button variant="secondary" className="h-14 px-8 rounded-2xl font-black text-xs uppercase tracking-widest border-2" icon={<X size={20}/>} onClick={() => navigate("/attendance")}>
            {t('common.cancel')}
          </Button>
        }
      />
      ...
      <motion.div 
        ...
        className="bg-emerald-600 p-8 rounded-[2.5rem] text-white flex items-center gap-8 shadow-2xl shadow-emerald-600/10"
      >
        ...
        <div className="space-y-2">
           <p className="font-black text-lg tracking-tight">{t('attendance.file_format')}: <code className="bg-white/10 px-3 py-1 rounded-lg ml-2 border border-white/10">student_id, date, status</code></p>
           <p className="text-white/60 text-[10px] font-black uppercase tracking-widest leading-none">status: present | absent | excused | late</p>
        </div>
      </motion.div>
          ...
          {step === 2 && preview && (
            ...
            >
              <div className="flex items-center justify-between">
                 <h3 className="text-2xl font-black text-[var(--color-text-primary)] tracking-tight">2. {t('attendance.preview_title')} — {preview.length} {t('common.records', 'yozuv')}</h3>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[var(--color-bg-primary)]/50 border-b border-[var(--color-border)]">
                      ...
                      <th className="px-8 py-5 text-[10px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest text-center">{t('attendance.status')}</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={i} className="hover:bg-[var(--color-bg-primary)]/40 transition-colors">
                        ...
                        <td className="px-8 py-5 text-center">
                           <span 
                             ...
                             style={{ color: statusColor(row.status), backgroundColor: `${statusColor(row.status)}10`, borderColor: `${statusColor(row.status)}20` }}
                           >
                             <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: statusColor(row.status) }} />
              ...
              <div className="pt-10 border-t border-[var(--color-border)] flex justify-between gap-6">
                 ...
                 <Button 
                   ...
                   icon={<Save size={24} />}
                 >
                   {loading ? t('common.saving') : t('attendance.save_all')}
                 </Button>
              </div>
            </motion.div>
          )}
